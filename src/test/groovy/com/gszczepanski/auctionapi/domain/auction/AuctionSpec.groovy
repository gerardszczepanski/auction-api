...
class AuctionSpec extends Specification implements WithAuctionFixtures {
    ...
    def "should create Auction from CreateAuctionSpecification"() {
        given:
            CreateAuctionSpecification specification = givenCreateAuctionSpecification()
            Time time = Stub()
            time.now() >> specification.getStartDate().minusHours(1)

        when:
            Auction auction = createFrom(specification, time)
    ...
    def "should throw exception on create Auction from CreateAuctionSpecification when Time is null"() {
        given:
            CreateAuctionSpecification specification = givenCreateAuctionSpecification()

        when:
            createFrom(specification, null)
